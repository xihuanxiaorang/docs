...
数据绑定的一个常见需求场景是操纵元素的 CSS class 列表和 style 内联样式。因为 `class` 和 `style` 都是 attribute，所有我们可以和其他 attribute 一样使用 `v-bind` 将它们和动态的字符串绑定。但是，在处理比较复杂的绑定时，通过拼接生成字符串是麻烦且易出错的。因此，Vue 专门为 `class` 和 `style` 的 `v-bind` 用法提供了特殊的功能增强。除了**字符串**外，表达式的值也可以是**对象**或**数组**。
