...
使用 `pnpm create vue@latest` 命令创建项目，项目名称为 `todomvc`。<br />![image-20240320230756876](https://cdn.jsdelivr.net/gh/xihuanxiaorang/img/202403202307171.png)

在项目被创建后，通过以下步骤安装依赖并启动开发服务器，如下所示：<br />![image-20240320231017934](https://cdn.jsdelivr.net/gh/xihuanxiaorang/img/202403202310537.png)
...
1. 首先，删除或者清空所有不需要的内容；
   ...
   3. 删除 `stores` 目录下的 `counter.js` 文件；
...
<template>
  ...
  <section class="main">
    ...
    <ul class="todo-list" v-for="(todo, index) in todos" :key="todo.id">
      <!-- These are here just to show the structure of the list items -->
      <!-- List items should get the class `editing` when editing and `completed` when marked as completed -->
      <TodoItem v-model="todos[index]"></TodoItem>
    </ul>
  </section>
</template>
...
```vue [TodoItem.vue] {2,8-39,46,49-57}
<script setup>
...
const doneEdit = () => {
  if (editing.value) {
    editing.value = false
    if (!editingTitle.value) return removeTodo(todo)
...
```js [todos.js] {9,14-41,81-86,90-92,96}
import { defineStore } from 'pinia'
...
<template>
  ...
  <section class="main" v-show="todos.length">
    ...
    />
    <label for="toggle-all">Mark all as complete</label>
    <ul class="todo-list" v-for="(todo, index) in filteredTodos" :key="todo.id">
      <!-- These are here just to show the structure of the list items -->
      <!-- List items should get the class `editing` when editing and `completed` when marked as completed -->
      <TodoItem v-model="filteredTodos[index]"></TodoItem>
    </ul>
  </section>
</template>
...
const route = useRoute()
const { activeTodos, completedTodos, todos } = storeToRefs(useTodosStore())
...
<template>
  ...
  <footer class="footer" v-show="todos.length">
    ...
    <!-- Hidden if no completed items are left ↓ -->
    <button class="clear-completed" v-if="completedTodos.length" @click="clearCompleted">
      Clear completed
    </button>
  </footer>
</template>
...
- 在启动项目时，先从本地存储 `LocalStorage` 中读取先前已存储的待办事项，如果读取不到则初始化为空数组；
- 使用存储对象的 `$subscribe` 方法监听 store 的变化，每当待办事项列表发生变化时，就将其最新数据保存到本地存储 `LocalStorage` 中。

::: code-group

```vue [TodoView.vue] {5,7-13}
<script setup>
import TodoHeader from '@/components/TodoHeader.vue'
import TodoMain from '@/components/TodoMain.vue'
import TodoFooter from '@/components/TodoFooter.vue'
...
/**
 * 使用$subscribe监听store变化，将最新的待办事项列表保存到本地存储中
 */
